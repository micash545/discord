...
const ENABLE_DEVTOOLS = _buildInfo.default.releaseChannel === 'stable' ? settings.get('DANGEROUS_ENABLE_DEVTOOLS_ONLY_ENABLE_IF_YOU_KNOW_WHAT_YOURE_DOING', false) : true;
let mainWindow = null;
...
function launchMainAppWindow(isVisible) {
  ...
  includeOptionalModule('./ElectronTestRpc', module => module.initialize(mainWindow)); // Electron has a bug where it clamps the window size to the primary display's size
  ...
  mainWindow.webContents.on('render-process-gone', (e, details) => {
    ...
    console.error(`[WebContents] crashed (reason: ${reason}, exitCode: ${details === null || details === void 0 ? void 0 : details.exitCode})... reloading`); // Optionally avoid automatic restarts which can make debugging crashes more difficult.
...
function includeOptionalModule(path, cb) {
  try {
    ...
    if (e && e.code === 'MODULE_NOT_FOUND') {
      console.log(`Module ${path} was not included.`);
    } else {
